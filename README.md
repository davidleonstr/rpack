...
builder = RPackBuilder(
    inputPath='./my-resources',
    outputPath='resources.rpack',
...
if pack.exists('config.json'):
    configData = pack.get('config.json')
...
```bash
# Using folder
python -m rpack create --input <input-directory> --output <output-file> [OPTIONS]

# Using files
python -m rpack create -f <input-files-separated-with-space> -o <output-file>
...
```bash
python -m rpack list <package-file> [OPTIONS]
...
```bash
python -m rpack extract <package-file> [OPTIONS]
...
builder = RPackBuilder(
    inputPath='./resources',      # Source directory or file
    outputPath='output.rpack',     # Destination package file
...
**Parameters:**
- `inputPath` (str): Path to source directory or file
- `outputPath` (str): Path for the output package file
...
# Get file contents
data = pack.get('file.txt', verifyHash=True)
...
# Check type
isFile = pack.isfile('config.json')
isDir = pack.isdir('assets')
...
**Methods:**
- `get(path, verifyHash=False)`: Retrieve file contents as bytes
...
data = pack.get('important.dat', verifyHash=True)
```
...
```python
# Read binary data
imageData = pack.get('images/logo.png')

# Save to file
with open('extractedLogo.png', 'wb') as f:
    f.write(imageData)
```
